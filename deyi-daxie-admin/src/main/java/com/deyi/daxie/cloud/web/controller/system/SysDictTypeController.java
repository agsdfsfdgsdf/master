...
import com.deyi.daxie.cloud.common.core.domain.entity.SysDictType;
...
public class SysDictTypeController extends BaseController {
    ...
    @ApiOperation("数据字典信息列表")
    @PreAuthorize("@ss.hasPermi('system:dict:list')")
    @GetMapping("/list")
    @Log(title = "字典类型->数据字典信息列表", businessType = BusinessType.SEARCH)
    ...
    @ApiImplicitParams({
            @ApiImplicitParam(name = "dictIds", value = "字典id", required = true, dataType = "Long")
    })
    @PreAuthorize("@ss.hasPermi('system:dict:query')")
    @GetMapping(value = "/{dictId}")
    @Log(title = "字典类型->查询字典类型详细", businessType = BusinessType.SEARCH)
    ...
    @Log(title = "字典类型->获取字典选择框列表", businessType = BusinessType.SEARCH)
